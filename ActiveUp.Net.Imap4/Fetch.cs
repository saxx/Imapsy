...
namespace ActiveUp.Net.Mail
{
    ...
    public class Fetch
    {
		...
		public string HeaderString(int messageOrdinal)
		{
            ...
            // I had wierd exceptions here from e-mails that had an empty header
            try
            {
	    ...
	    private static string ExtractMessageFromReponse(string response)
	    {
            ...
            // FIX by Hannes Sachsenhofer for Imapsy
            // Sometimes the message-size is to big for the entire message and the Substring(...) fails because of that
	        var message = "";
            try
            {
                message = response.Substring(messageStart, messageSize);
            ...
            {
                message = response.Substring(messageStart);
